 ...
 * @example 
// 模块功能通过 AudioMgr 调用
AudioMgr.playMusic("audios/nocturne");
 */
export class AudioMgr extends Component {
    ...
    url: string = '';
    playMusic(url: string, callback?: Function, loop: boolean = true) {
        this.url = url;
    ...
    save() {
        ...
        StorageMgr.set(LOCAL_STORE_KEY, data);
    }
    ...
    load() {
        ...
        let data = StorageMgr.get(LOCAL_STORE_KEY);
        if (data) {
            try {
